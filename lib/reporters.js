...
const RETRYABLE_ERROR_CODES = new Set(["ECONNREFUSED", "ECONNRESET", "ENOTFOUND", "EPIPE", "ETIMEDOUT"]);
...
class RetryHttp extends datadogApiClient.client.IsomorphicFetchHttpLibrary {
    ...
    retryDelay(response, tryCount) {
        if (response.httpStatusCode === 429) {
            ...
            const delayHeader = response.headers["retry-after"] || response.headers["x-ratelimit-reset"];
...
class DatadogReporter {
    ...
    constructor(options = {}) {
        ...
        this.site =
            options.site || process.env.DATADOG_SITE || process.env.DD_SITE || process.env.DATADOG_API_HOST;
